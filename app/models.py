...
class UserSession(db.Model):
        ...
        id = db.Column(db.String(100),primary_key = True)
        user_id = db.Column(db.String(100),db.ForeignKey('user_master.id'))
...
class QuizMaster(db.Model):
        ...
        id = db.Column(db.String(100),primary_key = True)
        quiz_name = db.Column(db.String(500), unique = True)
        is_active = db.Column(db.Integer, default = 1)
        created_ts = db.Column(db.DateTime, default = datetime.utcnow)
        updated_ts = db.Column(db.DateTime)
...
class QuizQuestions(db.Model):
        ...
        id = db.Column(db.String(100),primary_key = True)
        quiz_id = db.Column(db.String(100),db.ForeignKey('quiz_master.id'))
        question_id = db.Column(db.String(100),db.ForeignKey('question_master.id'))
        is_active = db.Column(db.Integer, default = 1)
        created_ts = db.Column(db.DateTime, default = datetime.utcnow)
        updated_ts = db.Column(db.DateTime)
...
class QuizInstance(db.Model):
        ...
        def __init__(self, id, quiz_id, user_id, score_achieved, is_submitted):
            self.id = id
            self.quiz_id = quiz_id
            self.user_id = user_id
